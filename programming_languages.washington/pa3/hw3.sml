...
(* using List.find that finds exactly the first occurrence
   wrong types: ('a -> 'b option) -> 'a list -> 'a list *)
(*
fun first_answer2 is_wanted lst =
    let val selector = fn el => case is_wanted(el) of
                                    NONE   => false
                                  | SOME v => true
    in
        case List.find selector lst of
            NONE   => raise NoAnswer
          | SOME v => v (* gives -> 'a *)
(*          | SOME v => [v] *) (* gives -> 'a list *)
    end
*)
...
(* TODO: incorrect types ('a -> 'b option) -> 'a list -> 'a list option
  | x::xs' => test_and_collect(xs', acc@[x],
                               (* if x does not pass the check, all_good becomes false *)
                               if isSome(is_wanted x) then all_good else false)
*)
