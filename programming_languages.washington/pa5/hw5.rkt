...
;; works but does not comply to the spec
;; should not use eval-under-env :-(
;; should not use helper function :-(
;(define (mlet*-ok lstlst e2)
;  (define (inner-mlet* lst acc-env)
;    (if (null? lst)
;        (eval-under-env e2 acc-env)
;        (inner-mlet* (cdr lst)
;                     (cons (cons (car (car lst))
;                                 (eval-under-env (cdr (car lst)) acc-env))
;                           acc-env))))
;    (inner-mlet* lstlst null))

;; works but...
;; mupl/racket mix
;(define (mlet*-mix lstlst e2)
;  (letrec ([inner-mlet*
;            (lambda (lst acc-env)
;              (if (null? lst)
;                  (eval-under-env e2 acc-env)
...
;; ok, but a bit awkward
;; using simple mlet
;(define (ifeq-2 e1 e2 e3 e4)
;  (mlet "_x" e1
;        (mlet "_y" e2
;              (ifgreater (var "_x") (var "_y")
;                         e4
